...
# averaged 1h files into monthly means myself and then made climatological mean
#u=np.load('gom3_ua_mean.npy') 
#v=np.load('gom3_va_mean.npy')

#GOM3_DATA_MonthlyMeanClimat # monthly means preaveraged by Chen
#u=np.load('GOM3_DATA_MonthlyMean/gom3_ua_monthly_mean_201304.npy').flatten()  # individual files downloaded
#v=np.load('GOM3_DATA_MonthlyMean/gom3_va_monthly_mean_201304.npy').flatten()  # np.array(ds['ua']).shape (1, 90415)

#FNU='gom3_ua_mean_climat_19782016.npy';u=np.load(FNU)
#FNV='gom3_va_mean_climat_19782016.npy';v=np.load(FNV)
#FNU='gom3_ua_monthly_mean_climat_19782016_m04.npy';u=np.load(FNU) # specific month
#FNV='gom3_va_monthly_mean_climat_19782016_m04.npy';v=np.load(FNV)
#FNU='gom3_us_mean_climat_19782016.npy';u=np.load(FNU)
#FNV='gom3_vs_mean_climat_19782016.npy';v=np.load(FNV)

# JiM wants to plot a hourly field from say April 7th, 2013
import netCDF4
from datetime import datetime as dt
dtime=dt(2013,4,7,0,0,0)
...
#i=np.arange(len(Grid['lonc'])) # whole domain
kfGB=np.argwhere((Grid['lonc']>-70.)&(Grid['lonc']<-65.)&(Grid['latc']>39.5)&(Grid['latc']<42.5)).flatten() # Georges Bank
LON1=-71.;LON2=-64.+1.e-6;LAT1=39.0;LAT2=44.+1.e-6
loni=np.arange(-69.6,-65.0001,0.02)
lati=np.arange( 39.5, 43.0001,0.02)
loni=np.arange(LON1,LON2,0.02)
lati=np.arange(LAT1,LAT2,0.02)
#loni=np.arange(-68.5,-68.0001,0.01)
#lati=np.arange( 41.3, 41.8001,0.01)
...
#sp=np.array([[-69,-68,-67,-66],[41.5,41.5,41.5,41.5]]).T
...
#plt.figure() 
#plt.quiver(lonii, latii, urcii, vii,color='r')
#plt.show()
SpawnNEP=np.array([
       [-67.18, 41.57],
       [-66.10, 41.57],
...
plt.tricontour(Grid['lon'],Grid['lat'],Grid['h'],[10.,70.,100.,200.],colors='k')  
strm=plt.streamplot(lonii,latii,urcii,vii,density=4,color=np.sqrt(uii*uii+vii*vii),cmap='GnBu')
fig.colorbar(strm.lines)
S=SpawnNEP;i=np.arange(-1,len(S))
plt.plot(S[i,0],S[i,1],'g-')
...
plt.figure()
plt.streamplot(lonii,latii,urcii,vii,start_points=sp,integration_direction='forward',color=np.sqrt(uii*uii+vii*vii),cmap='GnBu')
plt.title(FNU)
plt.show()
